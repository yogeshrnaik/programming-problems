...
public class CandidateCode
{
    ...
    private static class Pot {
        ...
        public String toString() {
            return "Size = " + potSize + ",noOfStonesInPot = " + noOfStonesInPot + ", largestPotLessThanMe: "+ (largestPotLessThanMe != null ? largestPotLessThanMe.potSize : null)
    ...
    private static void putStones(Pot currentPot, int stonesToAdd) {
        ...
        int stonesRequiredIfCurrentPotIsLargestPot = currentPot.possiblePots.get(currentPot.possiblePots.size()-1) - currentPot.noOfStonesInPot;
        ...
        if (difference > currentPot.possiblePots.size() * stonesRequiredToOverflowSmallestPotNotYetOverflown) {
            // ------ recursive call - put stones = smallest not overflown pot's size minus noOfStonesInPot into the largest pot < currentPot
            Pot smallestPotNotYetOverFlown = findSmallestPotNotYetOverflown();
            ...
            // ---- else if no of stones required to overflow the currentPot <= diff (extra condition to avoid adding stones more than required to overflow the pot)
            // ------- put the stones required to overflow in currentPot
